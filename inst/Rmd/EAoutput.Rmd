...
knitr::kable(params$standard.CN[12], format = "simple")

# #standards.mean <- as_tibble(params$standard.CN)  %>%
#                     group_by(group)  %>%
#                     summarise(d13C.raw = mean(d.13C.12C.raw),
#                               d13C.VPDB = mean(d.13C.12C.VPDB),
#                               d15N.raw = mean(d.15N.14N.raw),
#                               d15N.air = mean(d.15N.14N.air))
...
# ZEROS & BLANKS
The table below includes measurable blanks and zeros. Blanks are empty tins while zeros are no tin or sample. The table below, if given, contains the data for blanks and zero from this run. No table indicates blanks and zeros were not measurable. Blank correction has not been implemented in the script.  
```{r echo=FALSE}
...
#params$standard.plots[1]
#paramsstandard.plots[2]
