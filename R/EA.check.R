...
EA.check <- function(results){
  ...
  mean.salmon.pctC.measured <- mean(standard.CN$mass.percent.C[standard.CN$group == "SALMON"], na.rm = T)

  mean.GA1.d15N.measured <- mean(standard.CN$d.15N.14N.air[standard.CN$group == "GA1"], na.rm = T)
  mean.GA1.pctN.measured <- mean(standard.CN$mass.percent.N[standard.CN$group == "GA1"], na.rm = T)

  #Calculate accuracy for d13C, d15N, pctC, and pctN. Use salmon or peach leaves standard for C and GA1 for N.
  error.analysis.results$Accuracy[error.analysis.results$Value == "d13C_VPDB"] <- mean.salmon.d13C.measured - known.standard.values["SALMON", "d13C_VPDB"]
  error.analysis.results$Accuracy[error.analysis.results$Value == "mass.percent.C"] <- mean.salmon.pctC.measured - known.standard.values["SALMON", "mass.percent.C"]


  error.analysis.results$Accuracy[error.analysis.results$Value == "d15N_air"] <- mean.GA1.d15N.measured - known.standard.values["GA1", "d15N_air"]
  error.analysis.results$Accuracy[error.analysis.results$Value == "mass.percent.N"] <- mean.GA1.pctN.measured - known.standard.values["GA1", "mass.percent.N"]
  ...
  error.analysis.results$Precision[error.analysis.results$Value == "mass.percent.C"] <- sd(standard.CN$mass.percent.C[standard.CN$group == "SALMON"], na.rm = T)
  ...
  error.analysis.results$Precision[error.analysis.results$Value == "mass.percent.N"] <- sd(standard.CN$mass.percent.N[standard.CN$group == "GA1"], na.rm = T)
